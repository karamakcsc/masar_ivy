...
frappe.ui.form.on('Balance Confirmation Statement', {
	refresh: function(frm) {
		...
		frm.add_custom_button('Prepare Statements',function(){
			...
			frappe.call({
				method: "masar_ivy.masar_ivy.doctype.balance_confirmation_statement.balance_confirmation_statement.prepare_statements",
				args: {
	...
	fetch_customers: function(frm){
		if(frm.doc.collection_name){
			frappe.call({
				method: "masar_ivy.masar_ivy.doctype.balance_confirmation_statement.balance_confirmation_statement.fetch_customers",
				args: {
	...
	download_statements: function(frm) {
		// Construct the API URL for the download
		const url = frappe.urllib.get_full_url(
			'/api/method/masar_ivy.masar_ivy.doctype.balance_confirmation_statement.balance_confirmation_statement.download_statements' //api url
		) + '?download_link=' + encodeURIComponent(frm.doc.download_link); // download link
