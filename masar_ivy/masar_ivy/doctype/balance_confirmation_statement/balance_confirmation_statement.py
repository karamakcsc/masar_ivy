...
from erpnext.accounts.report.general_ledger.general_ledger import get_result as get_soa
...
@frappe.whitelist()
def prepare_statements_from_omar(docname):
...
def prepare_statements(docname):
	...
	customers = [customer.customer for customer in doc.customers]
	#frappe.msgprint(str(len(customers)))
	if customers:
		if len(customers) > 30:
			frappe.enqueue(get_customers_statements, timeout=1500, doc=doc, site=site)
			doc.reload()
		else:
			get_customers_statements(doc, site, publish_progress=True)
			# since this method is called via frm.call this doc needs to be updated manually
			doc.reload()


def get_customers_statements(doc, site, publish_progress=True):
	#frappe.init(site=site)
	#frappe.connect()
	full_path = site + "/public/files/balance-statements/" + doc.name
	output_file = "/public/files/balance-statements/" + doc.name
	...
	for customer in doc.customers:
		...
		count+=1
		if publish_progress:
			frappe.publish_progress(count*100/len(doc.customers),
				title = _("Creating Balance Confirmation Report ..."))
		...
		filters= frappe._dict({
			'from_date':doc.to_date,  #date(2000, 1, 1),
			'to_date': doc.to_date,
			'company': doc.company,
			'finance_book':  None,
			'account':  None,
		...
		res = get_soa(filters,account_details=None)
		context = {
			"doc": doc,
...
def get_customers_statements_from_omar(doc, site):
	frappe.init(site=site)
	frappe.connect()
